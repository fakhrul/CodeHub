...
namespace CodeHub.ViewControllers.Gists
{
    public class GistCreateViewController : DialogViewController
    {
        ...
        public override void ViewDidLoad()
        {
            ...
            OnActivation(d =>
            {
                saveButton.GetClickedObservable().InvokeCommand(ViewModel.SaveCommand).AddTo(d);
                ViewModel.WhenAnyValue(x => x.Description).Subscribe(_ => UpdateView()).AddTo(d);
                ViewModel.WhenAnyValue(x => x.Files).Subscribe(_ => UpdateView()).AddTo(d);
                ViewModel.WhenAnyValue(x => x.Public).Subscribe(_ => UpdateView()).AddTo(d);
