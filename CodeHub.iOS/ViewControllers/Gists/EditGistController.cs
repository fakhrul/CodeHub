...
namespace CodeHub.ViewControllers.Gists
{
    public class EditGistController : DialogViewController
    {
        ...
        public override void ViewDidLoad()
        {
            ...
            OnActivation(d =>
            {
                cancelButton.GetClickedObservable().Subscribe(_ => Discard()).AddTo(d);
                saveButton.GetClickedObservable().Subscribe(_ => Save().ToBackground()).AddTo(d);
                _descriptionElement.Clicked.Subscribe(_ => ChangeDescription()).AddTo(d);
                _addFileElement.Clicked.Subscribe(_ => AddFile()).AddTo(d);
            });
        }
