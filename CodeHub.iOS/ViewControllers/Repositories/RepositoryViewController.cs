...
namespace CodeHub.Views.Repositories
{
    public class RepositoryViewController : PrettyDialogViewController<RepositoryViewModel>
    {
        ...
        public override void ViewDidLoad()
        {
            ...
            OnActivation(d =>
            {
                _stargazers.Clicked.InvokeCommand(ViewModel.GoToStargazersCommand).AddTo(d);
                _watchers.Clicked.InvokeCommand(ViewModel.GoToWatchersCommand).AddTo(d);
                _forks.Clicked.InvokeCommand(ViewModel.GoToForkedCommand).AddTo(d);
                actionButton.GetClickedObservable().Subscribe(_ => ShowExtraMenu()).AddTo(d);

                _eventsElement.Clicked.InvokeCommand(ViewModel.GoToEventsCommand).AddTo(d);
                _ownerElement.Clicked.InvokeCommand(ViewModel.GoToOwnerCommand).AddTo(d);

                _commitsElement.Clicked.InvokeCommand(ViewModel.GoToCommitsCommand).AddTo(d);
                _pullRequestsElement.Clicked.InvokeCommand(ViewModel.GoToPullRequestsCommand).AddTo(d);
                _sourceElement.Clicked.InvokeCommand(ViewModel.GoToSourceCommand).AddTo(d);

                ViewModel.WhenAnyValue(x => x.Branches).Subscribe(_ => Render()).AddTo(d);
                ViewModel.WhenAnyValue(x => x.Readme).Subscribe(_ => Render()).AddTo(d);

                _forkElement.Value.Clicked.Select(x => ViewModel.Repository.Parent).InvokeCommand(ViewModel.GoToForkParentCommand).AddTo(d);
                _issuesElement.Value.Clicked.InvokeCommand(ViewModel.GoToIssuesCommand).AddTo(d);
                _readmeElement.Value.Clicked.InvokeCommand(ViewModel.GoToReadmeCommand).AddTo(d);

                _websiteElement
                    .Value.Clicked.Select(x => ViewModel.Repository.Homepage)
                    .Select(x => new WebBrowserViewController(x))
                    .Subscribe(x => NavigationController.PushViewController(x, true))
                    .AddTo(d);

                HeaderView.Clicked.InvokeCommand(ViewModel.GoToOwnerCommand).AddTo(d);
        ...
        public RepositoryViewController(string username, string repository)
            : this()
        {
            ViewModel = new RepositoryViewModel(username, repository);
        }
