...
namespace CodeHub.ViewControllers.Source
{
    ...
    {
        private readonly IJsonSerializationService _serializationService = Locator.Current.GetService<IJsonSerializationService>();
        ...
        public override void ViewDidLoad()
        {
            ...
            ViewModel.WhenAnyValue(x => x.FilePath).Subscribe(x =>
            {
                var data = System.IO.File.ReadAllText(x, System.Text.Encoding.UTF8);
                var patch = System.Web.HttpUtility.JavaScriptStringEncode(data);
        ...
        public override bool ShouldStartLoad(WKWebView webView, WKNavigationAction navigationAction)
        {
            ...
            if(url.Scheme.Equals("app")) {
                ...
                {
                    var data = System.Web.HttpUtility.UrlDecode(url.Fragment);
                    var commentModel = _serializationService.Deserialize<JavascriptCommentModel>(data);
